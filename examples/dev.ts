...
import { dot, matmul_acc, mul_acc, sign, sub } from "../src/base/raw_tensor_operations.ts";
...
try {
    ...
    const nn = weight.matmul(input).add(bias).logistic().set_name("x").mse_loss(target);
